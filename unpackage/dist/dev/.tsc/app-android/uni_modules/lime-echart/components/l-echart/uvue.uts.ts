...
export class Echarts {
	...
	init(){
		...
		this.context.addEventListener('message', (e : UniWebViewMessageEvent) => {
			...
			if (key == 'log' && data != null) {
				__f__('log','at uni_modules/lime-echart/components/l-echart/uvue.uts:29',data)
			}
	...
	on(type:string, query: any, callback: EchartsEventHandler) {
		...
		__f__('warn','at uni_modules/lime-echart/components/l-echart/uvue.uts:94','uvue 暂不支持事件')
	}
	on(type:string, callback: EchartsEventHandler) {
		...
		__f__('warn','at uni_modules/lime-echart/components/l-echart/uvue.uts:102','uvue 暂不支持事件')
	}
