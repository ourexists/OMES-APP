...
import { get, isEmpty, isString, parsePt, parseToObject } from "../../cool";
import type { ClFormLabelPosition, ClFormRule, ClFormValidateError } from "../../types";
import { $t, t } from "../../locale";
import { usePage } from "../../hooks";

type PassThrough = { __$originalPosition?: UTSSourceMapPosition<"PassThrough", "uni_modules/cool-unix/components/cl-form/cl-form.uvue", 82, 6>;
	className?: string;
};
...
return (): any | null => {
  ...
  return _cE("view", _uM({
    class: _nC(["cl-form", [_uM<string, any | null>({}),
			`cl-form--label-${labelPosition.value}`,
			_uM({
				'cl-form--disabled': disabled.value
			}),
			pt.value.className
		]])
  }), [
...
export type ClFormComponentPublicInstance = InstanceType<typeof __sfc__>;
const GenUniModulesCoolUnixComponentsClFormClFormStyles = [_uM([["cl-form", _pS(_uM([["width", "100%"]]))]])]
