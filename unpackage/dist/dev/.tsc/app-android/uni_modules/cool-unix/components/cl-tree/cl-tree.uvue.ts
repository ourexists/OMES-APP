...
import { computed, watch, ref, getCurrentInstance, provide, type PropType } from "vue";
import type { ClTreeItem, ClTreeNodeInfo } from "../../types";
import { first, isEmpty, isEqual, parsePt } from "../../cool";

type PassThrough = { __$originalPosition?: UTSSourceMapPosition<"PassThrough", "uni_modules/cool-unix/components/cl-tree/cl-tree.uvue", 71, 6>;
	className?: string;
};
...
// 向所有层级的子节点提供树实例，避免递归子项中 useParent 拿不到 cl-tree 导致选中样式不生效
const treeInstance = getCurrentInstance()?.proxy;
provide("ClTree", treeInstance);
...
const _component_cl_tree_item = resolveEasyComponent("cl-tree-item",_easycom_cl_tree_item)
  ...
  return _cE("view", _uM({
    class: _nC(["cl-tree", [_uM<string, any | null>({}),pt.value.className]])
  }), [
    _cE(Fragment, null, RenderHelpers.renderList(data.value, (item, __key, __index, _cached): any => {
      return _cV(_component_cl_tree_item, _uM({
        key: item.id,
        item: item,
        level: 0,
        pt: props.pt
      }), null, 8 /* PROPS */, ["item", "pt"])
    }), 128 /* KEYED_FRAGMENT */)
  ], 2 /* CLASS */)
}
}
...
export type ClTreeComponentPublicInstance = InstanceType<typeof __sfc__>;
const GenUniModulesCoolUnixComponentsClTreeClTreeStyles = []
